...
contract("Identity: Create2", ([sender, receiver, thirdperson, fourthperson]) => {
  ...
  beforeEach(async function() {
    ...
    const {logs} = await this.identityFactory.createIdentity(sender, "Hello Metadata", {
      from: sender
    });
  ...
  it("Deployed a create2 contract", async () => {
    ...
    const {logs} = await identity.execute(2, sender, 0, accountBytecode, salt, {from: sender});
  ...
  it("It can deploy a create2 contract and call a function on it", async () => {
    ...
    const encodedCall = getEncodedCall(accountContract, "setOwner", [receiver]);

    const result = await identity.execute(0, accountContract.address, 0, encodedCall, 0, {
      from: sender
    });
